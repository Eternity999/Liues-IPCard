...
header("Content-type: image/JPEG"); //图片无法显示请注释此行查看报错
...
*/
imagettftext($dst_im, 14, 0, 95, 140, imagecolorallocate($dst_im, 0,51,78), $font, $weather); //今天天气添加到图片
imagettftext($dst_im, 13, 0, 160, 90, imagecolorallocate($dst_im, 0,51,78), $font, '温度:'.$temp.'℃'); //当前温度添加到图片
imagettftext($dst_im, 13, 0, 237, 90, imagecolorallocate($dst_im, 0,51,78), $font, '湿度:'.$humidity.'%RH'); //当前湿度添加到图片
imagettftext($dst_im, 11, 0, 160, 115, imagecolorallocate($dst_im, 0,51,78), $font, '风向:'.$winddirection.'方'); //当前风向添加到图片
imagettftext($dst_im, 11, 0, 237, 115, imagecolorallocate($dst_im, 0,51,78), $font, '风力级别:'.$windpower.'级'); //风力级别添加到图片
imagettftext($dst_im, 11, 0, 160, 135, imagecolorallocate($dst_im, 0,51,78), $font, '更新时间:'.$reporttime); //更新时间添加到图片
//输出
imagegif($dst_im);
imagedestroy($dst_im);
?>